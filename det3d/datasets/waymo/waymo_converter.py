...
def convert(idx):
    ...
    for frame_id, data in enumerate(dataset):
        ...
        # Retry mechanism for file operation
        retry_count = 0
        while True:
            try:
                ...
                if retry_count < 5:  # Retry for a maximum of 5 times
                    retry_count += 1
                    time.sleep(0.1)  # Wait for 0.1 seconds before retrying
                else:
                    print("Failed to write file after 5 retries:", e)
                    break  # If unsuccessful after 5 retries, exit the retry loop
